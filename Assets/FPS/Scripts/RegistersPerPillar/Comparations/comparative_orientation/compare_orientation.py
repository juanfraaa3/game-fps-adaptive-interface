...
file_path = "comparative_orientation.xlsx"

BASE_OUTPUT_DIR = r"C:\Users\Juanes\adaptative_microgame\Assets\FPS\Scripts\RegistersPerPillar\Comparations\comparative_orientation"
...
# En orientation todas continuas son "menor es mejor"
BIGGER_IS_BETTER = {}  # vacío
...
def pct_change(base, adap):
    if base == 0:
        return np.nan
    return (base - adap) / abs(base) * 100  # menor es mejor
...
GROUPS = {
    "Orientation_noobs": "_noob",
    "Orientation_regulars": "_regular",
    "Orientation_experts": "_expert",
    "Orientation_all": None
}
...
for folder_name, pattern in GROUPS.items():
    ...
    for col in ORIENTATION_COLUMNS:
        ...
        delta = med_a - med_b
        pct = pct_change(med_b, med_a)

        lo, hi = bootstrap_ci_delta(xb, ya)

        direction = "Mejora descriptiva" if med_a < med_b else ("Empeora" if med_a > med_b else "Sin cambio")
...
print("\nAnalisis ORIENTATION completo guardado por carpetas.")